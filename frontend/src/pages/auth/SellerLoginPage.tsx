...
export default function SellerLoginPage({ onLogin }: SellerLoginPageProps) {
  ...
  const getOrgSlug = () => {
    ...
    // Si la URL es /:orgSlug/seller-login, el slug está en pathParts[1]
    if (pathParts.length > 1 && pathParts[1] && pathParts[1] !== 'seller-login') {
      return pathParts[1];
    }
    return null;
  };
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (user) {
      onLogin(user.id, user.name || 'Vendedor');
      
      // Navegar con el prefijo de la organización
